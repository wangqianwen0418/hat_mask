...
check_point = keras.callbacks.ModelCheckpoint("resnet_model/resnet_model.h5", monitor='val_loss', verbose=0, save_best_only=True, save_weights_only=False)
tf_log = keras.callbacks.TensorBoard(log_dir='resnet_model/tf_logs', batch_size=batch_size, write_graph=True)
...
x = Flatten()(x)
x = Dense(256, activation='relu')(x)
total_output = Dense(max_count, activation='softmax')(x)
